...
proc map_chromosome(c:TChromosome):TAgent =
  ...
  agent = (
    filterCount:   linear_scale(c.Chromo[0],2,150),
    kernelSize:    linear_scale(c.Chromo[1],2,6),
    poolingWindow: linear_scale(c.Chromo[2],5,50),
    denseSize1:    linear_scale(c.Chromo[3],10,100),
    denseSize2:    linear_scale(c.Chromo[4],10,100),
    dropout1:                   c.Chromo[5],
    dropout2:                   c.Chromo[6],
    batchSize:     linear_scale(c.Chromo[7],5,300),
    epochCount:    linear_scale(c.Chromo[8],10,150) 
  )
  return agent
...
proc execute_agent(agent:TAgent, generation:int, popIndex:int,output_dir:string):float=
  ...
  let retval = execShellCmd("python exp-07.py " & 
                              $(agent.filterCount) & " "  &
                              $(agent.kernelSize) & " " &
                              $(agent.poolingWindow) & " " &
                              $(agent.denseSize1) & " " &
...
proc main() =
  ...
  for g in 0..GenerationCount-1:
    ...
    echo "[",getGmTime(getTime()), "]Best fitness =", max_fitness, 
          " output7-", intToStr(g,4),"-", intToStr(max_index,4), ".txt ", 
          best_agent

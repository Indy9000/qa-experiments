...
# --------------------------------------------------------------------------------------------
proc parse_output1(filename:string) : (float,float,float,float,float) =
    #Line to look for an parse
    #26s - loss: 0.3458 - acc: 0.8907 - val_loss: 0.3320 - val_acc: 0.896
    let token1="s - loss: "
    let token2="Val Mean avg Prec = "
    var tr_loss,tr_acc,val_loss,val_acc,val_map : float
    for line in lines filename:
        let k1 = line.find(token1)
        if k1 > -1:
            let ll = line[(k1+len(token1)) .. ^1]
        ...
        if k2 > -1:
            let ll = line[(k2+len(token2)) .. ^1]
            val_map = parseFloat(ll)
    return (tr_loss,tr_acc,val_loss,val_acc,val_map)

# --------------------------------------------------------------------------------------------
proc compute_performance(tr_l:float, tr_a:float, va_l:float, va_a:float, va_map:float):(float,string) =
...
proc execute_agent(agent:TAgent, generation:int, popIndex:int,output_dir:string):float=
  ...
  let (tr_l,tr_a,va_l,va_a,va_map) = parse_output1(filename)
  let (fitness,data) = compute_performance(tr_l,tr_a,va_l,va_a,va_map)

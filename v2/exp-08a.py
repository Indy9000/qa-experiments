...
def vectorise_qa_lines_labels(qi,qs,qa_lines,labels,qas_list):
...
vectorise_qa_lines_labels(qi_tr, all_q, full_qa_lines, full_labels, tr_questions)
va_start_index = len(full_qa_lines)
vectorise_qa_lines_labels(qi_va, all_q, full_qa_lines, full_labels, va_questions)
te_start_index = len(full_qa_lines)
# Then add the test set to the big text array
vectorise_qa_lines_labels(qi_te, test_q, full_qa_lines, full_labels, te_questions)
...
###
for k in range(1,50):
    print(full_qa_lines[len(full_qa_lines)-k])
###
...
def ComputeMAP(qas,prediction,print_stats=False):
    qi = 0  # Question index
    ai = 0  # Answer index
    ...
    for jj in range(len(qas)):
        ...
        if print_stats:
            ...
            print("QID",qas[jj].QuestionIndex,"CorrectAnswers",qas[jj].CorrectAnswers,"PredictedList",qas[jj].PredictedList)
...
print("Computing MAP for test set")
te_map = ComputeMAP(te_questions, te_predictions, True)

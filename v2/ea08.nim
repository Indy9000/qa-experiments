...
proc map_chromosome(c:TChromosome):TAgent =
  ...
  agent = (
    ...
    denseSize:    linear_scale(c.Chromo[3],10,50),
    dropout:                   c.Chromo[4],
    batchSize:    linear_scale(c.Chromo[5],4,64),
    epochCount:   linear_scale(c.Chromo[6],10,50) 
  )
  return agent
# --------------------------------------------------------------------------------------------
...
proc execute_agent(agent:TAgent, generation:int, popIndex:int,output_dir:string):float=
  ...
  let retval = execShellCmd("python exp-08.py " &
                              $(agent.filterCount) & " "  &
                              $(agent.kernelSize) & " " &
                              $(agent.poolingWindow) & " " &
                              $(agent.denseSize) & " " &
...
proc main() =
  ...
  for g in 0..GenerationCount-1:
    ...
    echo "[",getGmTime(getTime()), "]Best fitness =", max_fitness,
          " output8-", intToStr(g,4),"-", intToStr(max_index,4), ".txt ",
          best_agent

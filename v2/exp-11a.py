...
# Computes the Mean Reciprocal Rangking of the prediction
...
def ComputeMRR_and_MAP(qas,prediction,qii,print_stats=False):
    qi = 0  # Question index
    ai = 0  # Answer index
    k=0
    ...
    for jj in range(len(qas)):
        ...
        if print_stats:
            ...
            print("QID",qas[jj].QuestionIndex,"CorrectAnswers",qas[jj].CorrectAnswers,"/",qas[jj].AnswerCount,
                  "PredictedList",qas[jj].PredictedList)
        ##############  MRR  ####################
        ...
        for (ai,pp) in qas[jj].PredictedList:
            if ai in qas[jj].CorrectAnswers:
                rr = 1.0/float(rank) #reciprocal rangking = 1/rank_of_correct_answer
                mean_rr += rr
        ...
        for (ai,pp) in qas[jj].PredictedList:
            ...
            rank += 1
        if len(qas[jj].CorrectAnswers)>0:
            ap /= float(len(qas[jj].CorrectAnswers))
        mean_ap += ap
        #########################################

    mean_rr /= float(len(qas))
    mean_ap /= float(len(qas))
    return (mean_rr,mean_ap)

print("\n\n\n\nComputing MRR and MAP for validation set")
va_mrr,va_map = ComputeMRR_and_MAP(va_questions, va_predictions, va_qii, True)
print("\n\n\n\nComputing MRR and MAP for test set")
te_mrr,te_map = ComputeMRR_and_MAP(te_questions, te_predictions, te_qii, True)
print("\n\n\n\nComputing MRR and MAP for training set")
tr_mrr,tr_map = ComputeMRR_and_MAP(tr_questions, tr_predictions, tr_qii)

print("Test MRR=", te_mrr, "MAP=", te_map)
print("Val MRR=", va_mrr, "MAP=",va_map)
print("Train MRR=", tr_mrr,"MAP=",tr_map)

...
# --------------------------------------------------------------------------------------------
proc parse_output1(filename:string) : (float,float,float,float,float,float,float) =
    #Line to look for an parse
    ...
    for line in lines filename:
        ...
        if k1 > -1:
            ...
            val_loss1 = val_loss2 #save prev value
            val_loss2 = val_loss  #get current value
        ...
        if k2 > -1:
            let ll = line[(k2+len(token2)) .. ^1]
            let split_tokens = splitWhitespace(ll)
            val_map = parseFloat(strip(split_tokens[2]))
        
        let k3 = line.find(token3)
        if k3 > -1:
            let ll = line[(k3+len(token3)) .. ^1]
            let split_tokens = splitWhitespace(ll)
            te_map = parseFloat(strip(split_tokens[2]))

        if val_loss2 > val_loss1 or val_acc2 < val_acc1:
          # possible overfit, scale the fitness by overfit_penalty
          overfit_penalty = 1.0 - 1000.0*abs((val_loss2 - val_loss1) * (val_acc1 - val_acc2))

        overfit_penalty = if overfit_penalty > 1.0: 1.0 else: overfit_penalty
        overfit_penalty = if overfit_penalty < 0.0: 0.0 else: overfit_penalty
    return (tr_loss,tr_acc,val_loss,val_acc,val_map,te_map,overfit_penalty)
...
proc execute_agent(agent:TAgent, generation:int, popIndex:int,output_dir:string):float=
  ...
  let retval = execShellCmd("python exp-11.py " &
                              $(agent.filterCount) & " "  &
                              $(agent.kernelSize) & " " &
                              $(agent.poolingWindow) & " " &
                              $(agent.denseSize1) & " " &
  ...
  let (tr_l,tr_a,va_l,va_a,va_map,te_map,ofp) = parse_output1(filename)
  let (fitness,data) = compute_performance(tr_l,tr_a,va_l,va_a,va_map,te_map)
  echo "Generation ",generation," ",popIndex," Fitness= ",fitness," ofp= ",ofp," ",data
  return fitness * ofp
...
proc main() =
  ...
  for g in 0..GenerationCount-1:
    ...
    echo "[",getGmTime(getTime()), "]Best fitness =", max_fitness,
          " output11-", intToStr(g,4),"-", intToStr(max_index,4), ".txt ",
          best_agent
